...
يستخدم المشروع خدمة FCM لإرسال إشعارات للمستخدمين.

**أ. تسجيل الجهاز (Client-Side):**

1.  **التهيئة عند تسجيل الدخول:** بعد نجاح تسجيل دخول المستخدم، تستدعي دالة `checkLoginStatus` في `js/auth.js` دالة `setupFCM`.
2.  **طلب الإذن:** تطلب `setupFCM` من المستخدم الإذن لعرض الإشعارات.
3.  **الحصول على التوكن:** في حال موافقة المستخدم، يتم استدعاء `getToken()` من مكتبة Firebase للحصول على توكن فريد للجهاز/المتصفح.
4.  **حفظ التوكن:**
    *   يتم تخزين التوكن في `localStorage` (`fcm_token`) لتجنب طلبه في كل مرة.
    *   يتم إرسال التوكن مع `user_key` الخاص بالمستخدم عبر طلب `POST` إلى الواجهة الخلفية (`/api/save-token`، والذي يتم التعامل معه عبر `api/tokens.js`).
5.  **الواجهة الخلفية:** تقوم `api/tokens.js` بحفظ التوكن في جدول `user_tokens` وربطه بالمستخدم.

**ب. استقبال الإشعارات:**

1.  **التطبيق في الخلفية/مغلق (Background):**
    *   يستقبل عامل الخدمة `firebase-messaging-sw.js` الإشعار.
    *   يقوم بعرض الإشعار باستخدام `self.registration.showNotification()`.
2.  **التطبيق في الواجهة (Foreground):**
    *   يستقبل المستمع `onMessage` في `js/auth.js` بيانات الإشعار.
    *   يقوم بعرض الإشعار مباشرة باستخدام `new Notification()`.

**ج. إرسال الإشعارات (Server-Side):**

1.  **نقطة النهاية:** يتم استخدام نقطة النهاية `api/send-notification.js` لإرسال الإشعارات.
2.  **الآلية:** عند الحاجة لإرسال إشعار (مثلاً، عند تحديث حالة طلب)، يقوم الخادم بالبحث عن توكن FCM الخاص بالمستخدم المستهدف في جدول `user_tokens`.
3.  **الإرسال:** يستخدم الخادم `firebase-admin` لإرسال رسالة إلى التوكن المحدد، والتي تصل إلى جهاز المستخدم.
