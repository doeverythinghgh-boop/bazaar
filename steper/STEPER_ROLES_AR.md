...
| | **البائع (Seller)** | • لا يوجد. | • لا يوجد. | • لا تظهر نافذة (رسالة انتظار فقط). |
| | **الأدمن (Admin)** | • تحكم كامل. | • تعديل مباشر. | **المحتوى:** نفس نافذة المشتري مع صلاحية التجاوز. |
| **2. تأكيد الطلب (Confirmed)** | **البائع (Seller)** | • قبول/رفض المنتجات.<br>• تأكيد التجهيز. | • **جلب:** المنتجات غير الملغاة.<br>• **حفظ:** حالة `confirmed` أو `rejected`. | **المحتوى:** جدول منتجات مع مفتاح تبديل (Toggle) لكل منتج (متوفر/غير متوفر).<br>**الإجراءات:**<br>• `تبديل`: لرفض المنتج (تحويله لـ Rejected).<br>• `زر تفعيل`: يؤكد المنتجات المتوفرة. |
...
| **4. التوصيل (Delivered)** | **المشتري (Buyer)** | • تأكيد الاستلام.<br>• إرجاع منتجات. | • **جلب:** المنتجات المشحونة.<br>• **حفظ:** `delivered` أو `returned`. | **المحتوى:** قائمة منتجات مع مربعات اختيار (محددة = استلام، غير محددة = إرجاع).<br>**الإجراءات:**<br>• `إلغاء تحديد`: يعتبر المنتج "مرتجع" (Returned).<br>• `زر تفعيل`: يغلق الطلب كـ "تم التوصيل". |
