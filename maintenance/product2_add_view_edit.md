...
5.  **`edit2_submit.js`**:
   - **اكتشاف التغييرات (Change Detection)**: يتم مقارنة البيانات المدخلة بالنسخة الأصلية في `productSession`. إذا لم يحدث تغيير، يتم منع الإرسال لتوفير الموارد.
...
6.  **`edit2_init.js`**:
   - **جلب البيانات**: يتم استخراج كافة البيانات من `productSession` (الاسم، الوصف، الرسالة، الملاحظات) وتوزيعها على الحقول فور تحميل الصفحة.


## 📊 تدفق البيانات (Data Flow)
1. تحميل الصفحة -> `edit2_init.js` يملأ النموذج من الجلسة.
2. `edit2_image.js` يجلب الصور الحالية من السحابة.
3. التعديل -> الضغط اللحظي للصور الجديدة.
4. الحفظ -> `edit2_submit.js` يتحقق من التغييرات -> رفع الصور -> تحديث السجل -> حذف القديم -> إخطار الإدارة.
...
6. **`view2_init.js`**: دالة التهيئة التي تدعم جلب البيانات من `ProductStateManager` (الحديث) أو `productSession` (الاحتياطي).
