...
- **`js/admin.js`**: يحتوي على المنطق الخاص بلوحة تحكم المسؤول (عرض وإدارة المستخدمين).
- **`js/seller.js`**: يحتوي على المنطق الخاص بلوحة تحكم البائع (إضافة/تعديل/عرض المنتجات).
...
- **`js/login-page.js`**: يحتوي على المنطق البرمجي الخاص بصفحة `login.html`.
...
- **`api/orders.js`**: لإدارة الطلبات الجديدة.
...
4.  **التحقق من كلمة المرور (إذا كانت مطلوبة)**:
    ...
    - إذا كانت كلمة المرور غير صحيحة، تتوقف العملية.
...
1.  **الواجهة الأمامية**: المستخدم يضغط على زر "المشتريات" في `login.html`.
2.  **طلب البيانات**:
...
1.  **الواجهة الأمامية**: المستخدم يضغط على زر "إتمام الشراء" في نافذة السلة المنبثقة.
2.  **تجميع البيانات**:
